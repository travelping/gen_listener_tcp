...
start_link(Name, Module, InitArgs, Options) ->
    gen_server:start_link(Name, ?MODULE,
                          [{'__gen_listener_tcp_mod', Module} | InitArgs],
                          Options).
...
init([{'__gen_listener_tcp_mod', Module} | InitArgs]) ->
    ...
    case Module:init(InitArgs) of
        {ok, {Port, Options}, ModState} ->
            ...
            info_report(Verbose,
                        [listening_started, {port, Port}, {lsock, ListenSocket}
                         | Options0]),
...
handle_info({inet_async, LSock, ARef, {ok, ClientSock}},
            #listener_state{verbose = Verbose, socket=LSock,
                            acceptor=ARef, mod=Module,
                            mod_state=ModState}=St) ->
    info_report(Verbose, [new_connection, {csock, ClientSock},
                          {lsock, LSock}, {async_ref, ARef}]),
    patch_client_socket(ClientSock, LSock),

    info_report(Verbose, [handling_accept,
                          {module, Module},
    ...
    try
        case Module:handle_accept(ClientSock, ModState) of
            {noreply, NewModState} ->
                {noreply,
                 create_acceptor(St#listener_state{mod_state=NewModState})};
            {noreply, NewModState, hibernate} ->
                {noreply,
                 create_acceptor(St#listener_state{mod_state=NewModState}),
                 hibernate};
        ...
        Type:Exception ->
            error_report(Verbose, [gen_listener_tcp, {action, handle_accept},
...
handle_info({inet_async, LSock, ARef, Error},
            #listener_state{verbose = Verbose,
                            socket=LSock, acceptor=ARef}=ListenerState) ->
    error_report(Verbose, [acceptor_error, {reason, Error},
                           {lsock, LSock}, {async_ref, ARef}]),
...
patch_client_socket(CSock, LSock) when is_port(CSock), is_port(LSock) ->
    ...
    {ok, Opts} = prim_inet:getopts(LSock, [active, nodelay, keepalive,
                                           delay_send, priority, tos]),
    ok = prim_inet:setopts(CSock, Opts),
    ok.

create_acceptor(St) when is_record(St, listener_state) ->
    create_acceptor(St#listener_state.socket,
                    St#listener_state.mod,
                    St#listener_state.mod_state,
                    St#listener_state.verbose).

create_acceptor(ListenSocket, Module, ModState, Verbose)
  when is_port(ListenSocket) ->
    {ok, Ref} = prim_inet:async_accept(ListenSocket, -1),

    error_logger:info_report(Verbose, waiting_for_connection),
    #listener_state{verbose = Verbose, socket=ListenSocket,
                    acceptor=Ref, mod=Module, mod_state=ModState}.
